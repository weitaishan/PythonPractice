...
# 第三种：index()方法检测字符串中是否包含子字符串str，如果指定beg（开始）和end（结束）范围，左开右闭区间。
# 则检查是否包含在指定范围内，如果包含子字符串返回开始的索引值，否则会报一个异常。如果不指定beg和end则默认在整个字符串中检查。
# 语法：str.index(str, beg=0, end=len(string))
astr = "bvnhingdhaaddfsfx"
print(astr.index('dd'))    # 结果是11
print(astr.index('aa', 6, 10))   # 结果会抛出一个异常

# 第四种：rindex()和index方法相比是从右往左开始查找，但是返回来的索引还是从左往右的索引位置
astr = "bvnhingdhaaddfsfx"
print(astr.rindex('dd'))   # 结果是11，是从右往左找，但是返回来的索引还是从左往右的索引位置
print(astr.index('aa', 6, 10))   # 结果会抛出一个异常